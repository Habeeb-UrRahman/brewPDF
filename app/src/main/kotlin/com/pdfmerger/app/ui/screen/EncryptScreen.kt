...
fun EncryptScreen(onBack: () -> Unit) {
    ...
    BrewScaffold(
        ...
        if (selectedUri == null) {
            ...
            BrewActionButton(
                text = if (isDone) "Done ✓" else if (isProcessing) "Encrypting…" else "Lock PDF",
                enabled = password.isNotEmpty() && password == confirmPassword && !isProcessing && !isDone,
                onClick = {
                    isProcessing = true
                    errorMessage = null
                    scope.launch {
